...
struct Message(Copyable, Movable, Defaultable, Deinitable, JsonDatum):
    ...
    def _decode_expected[origin: ImmOrigin](mut self, mut r: WireReader[origin]) raises DecodeError -> Bool:
        if r.pos + 9 > len(r.data):
            return False
        if r.load_u64() != UInt64(2480480018956772898):
            return False
        if Int(r.data[r.pos + 8]) != 58:
            return False
        r.pos += 9
        self.f_bool = read_bool_here(r)
        if r.pos + 11 > len(r.data):
            return False
        if r.load_u64() != UInt64(3707709792083911212):
            return False
        if Int(r.data[r.pos + 8]) != 50:
            return False
        if Int(r.data[r.pos + 9]) != 34:
            return False
        if Int(r.data[r.pos + 10]) != 58:
            return False
        r.pos += 11
        self.f_int32 = r.read_int_here()
        if r.pos + 11 > len(r.data):
            return False
        if r.load_u64() != UInt64(3923882574197695020):
            return False
        if Int(r.data[r.pos + 8]) != 52:
            return False
        if Int(r.data[r.pos + 9]) != 34:
            return False
        if Int(r.data[r.pos + 10]) != 58:
            return False
        r.pos += 11
        self.f_int64 = r.read_int_here()
        if r.pos + 13 > len(r.data):
            return False
        if r.load_u64() != UInt64(7020949531036885548):
            return False
        if r.load_u32_at(8) != UInt32(573847156):
            return False
        if Int(r.data[r.pos + 12]) != 58:
            return False
        r.pos += 13
        self.f_float64 = read_float_here(r)
        if r.pos + 12 > len(r.data):
            return False
        if r.load_u64() != UInt64(7598263560198038060):
            return False
        ...
        if r.pos + 12 > len(r.data):
            return False
        if r.load_u64() != UInt64(7813586346809106988):
            return False
        ...
        if Int(r.data[r.pos + 12]) != 58:
            return False
        r.pos += 13
        self.f_int32_2 = r.read_int_here()
        if r.pos + 14 > len(r.data):
            return False
        if r.load_u64() != UInt64(7598263560198038060):
            return False
        if r.load_u32_at(8) != UInt32(845113198):
            return False
        if Int(r.data[r.pos + 12]) != 34:
            return False
        if Int(r.data[r.pos + 13]) != 58:
            return False
        r.pos += 14

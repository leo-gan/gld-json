...
struct Pet1(Copyable, Movable, Defaultable, Deinitable, JsonDatum):
    var breed: String
    ...
    def encoded_len_at(self, options: EncodeOptions, depth: Int) -> Int:
        ...
        if pretty:
            n += 1
        n += encoded_string_len(self.breed)
        if pretty:
    ...
    def encode_to(self, mut w: WireWriter, options: EncodeOptions):
        ...
        first = False
        w.write_bytes("\"breed\":".as_bytes())
        if pretty:
            w.write_byte(Byte(32))
        w.write_string(self.breed)
        if pretty:
            w.pretty_depth -= 1
    ...
    def _decode_expected[origin: ImmOrigin](mut self, mut r: WireReader[origin]) raises DecodeError -> Bool:
        if r.pos + 8 > len(r.data) or Int(r.data[r.pos + 0]) != 34 or Int(r.data[r.pos + 1]) != 98 or Int(r.data[r.pos + 2]) != 114 or Int(r.data[r.pos + 3]) != 101 or Int(r.data[r.pos + 4]) != 101 or Int(r.data[r.pos + 5]) != 100 or Int(r.data[r.pos + 6]) != 34 or Int(r.data[r.pos + 7]) != 58:
            return False
        r.pos += 8
        self.breed = r.read_string_here()
